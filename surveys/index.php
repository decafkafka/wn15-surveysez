...
{#records exist - process
    ...
    echo '<div align="center">We have ' . $myPager->showTotal() . ' ' . $itemz . '!</div><br />';
	echo '<div align="center"><table width="640"><tr><td>Date Created</td><td>Title</td><td>Description</td><td>Creator\'s Name</td></tr>';
	while($row = mysqli_fetch_assoc($result))
	{# process each row
         echo '<tr><td>' . $row['DateAdded'] . '</td><td><a href="' . VIRTUAL_PATH . 'surveys/survey_view.php?id=' . (int)$row['SurveyID'] . '">' . dbOut($row['Title']) . '</a></td><td>'. $row['Description'] . '</td><td>' . $row['AdminName'] . '</td></tr>';
		 echo '</table></div>';
	}
